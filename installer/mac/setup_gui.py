"""
First-launch setup GUI for Zinnia Axion.
...
Writes config to ~/.telemetry-tracker/config.env
"""
...
# Pre-configured by admin at build time (written by build.py).
try:
    from installer.mac.build_config import BACKEND_URL as DEFAULT_BACKEND_URL
except ImportError:
...
def show_setup(on_complete: callable = None) -> None:
    ...
    tk.Label(root, text="Zinnia Axion Setup", font=("Helvetica", 16, "bold")).pack(
        pady=(15, 5)
    )
    tk.Label(root, text="Enter your name or employee ID to get started.").pack(pady=(0, 10))

    frame = tk.Frame(root)
    frame.pack(pady=5)
    tk.Label(frame, text="User ID:", width=10, anchor="e").grid(row=0, column=0, padx=5)
    ...
    tk.Button(root, text="Save & Start", command=on_save, width=15).pack(pady=15)
